    ...
    public partial class ManageReservationsForm : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // FIXED validation: compare only date, not time
                if (dateIn.Date < DateTime.Now.Date)
                {
                    MessageBox.Show("The Date must be greater or equal to today's date",
                        "Invalid Date In", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else if (dateOut.Date < dateIn.Date)
                {
                    MessageBox.Show("The DateOut must be greater or equal to DateIn",
                        "Invalid Date Out", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
                    if (reservation.MakeReservation(roomNumber, clientID, dateIn, dateOut))
                    {
                        ...
                        MessageBox.Show("Reservation made successfully!",
                            "Make Reservation", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        btnClearFields.PerformClick();
                    }
                    else
                    {
                        MessageBox.Show("ERROR - Reservation not added!",
                            "Make Reservation", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Make Reservation error",
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int clientID = Convert.ToInt32(tbClientID.Text);
                int roomNumber = Convert.ToInt32(dgvReservations.CurrentRow.Cells[1].Value);
                DateTime dateIn = dateTimePickerIN.Value;
                DateTime dateOut = dateTimePickerOUT.Value;
                ...
                else
                {
                    if (reservation.EditReservation(reservationID, roomNumber, clientID, dateIn, dateOut))
                    {
                        ...
                        MessageBox.Show("Reservation updated successfully!",
                            "Edit Reservation", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("ERROR - Reservation not updated!",
                            "Edit Reservation", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
            catch (Exception ex)
